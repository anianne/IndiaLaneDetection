...
#test_images = [plt.imread(path) for path in glob.glob('test_images/*.jpg')] #Pi
#test_images = [plt.imread('solidWhiteRight.jpg')]

#show_images(test_images) #Pic
...
#show_images(list(map(select_rgb_white_yellow, test_images)))#Pic
...
#show_images(list(map(convert_hsv, test_images)))#Pic
...
#show_images(list(map(convert_hls, test_images))) #Pic
...
#white_yellow_images = list(map(select_white_yellow, test_images))#Pic

#show_images(white_yellow_images)#Pic
...
#gray_images = list(map(convert_gray_scale, white_yellow_images))#Pic

#show_images(gray_images)#Pic
...
#blurred_images = list(map(lambda image: apply_smoothing(image), gray_images))#Pic

#show_images(blurred_images)#Pic
...
#edge_images = list(map(lambda image: detect_edges(image), blurred_images))#Pic

#show_images(edge_images)#Pic
...
#roi_images = list(map(select_region, edge_images))#Pic

#show_images(roi_images)#Pic
...
#list_of_lines = list(map(hough_lines, roi_images))#Pic
...
#line_images = []#Pic
#for image, lines in zip(test_images, list_of_lines):#Pic
#    line_images.append(draw_lines(image, lines))#Pic
    
#show_images(line_images)#Pic
...
#lane_images = []#Pic
#for image, lines in zip(test_images, list_of_lines):#Pic
#    lane_images.append(draw_lane_lines(image, lane_lines(image, lines)))#Pic

    
#show_images(lane_images)#Pic
...
cap = cv2.VideoCapture('C:/anish/1science/2018-2019/original-india/car-finding-lane-lines-master/test_videos/solidWhiteRight.mp4')
...
while(cap.isOpened()):
    ...
    ret, frame = cap.read()
    if ret == True:
        cv2.imshow('Frame',(detector.process(frame)))
...
#os.chdir('C:/Users/IBM_ADMIN/Desktop/car-finding-lane-lines-master/output_videos')
